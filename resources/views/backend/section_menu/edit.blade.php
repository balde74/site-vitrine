		...
		<div class="x_content">
			<br />
			 <div class="col-md-2">
                ...
                <img src="{{ asset('documents/'.$sectionMenu->section_image) }}" alt="image de presentation" width="100%">
                @endif
            </div>

			<p></p>
			<form data-parsley-validate class="form-horizontal form-label-left col-md-10" method="POST" action="{{ route('section-menu.update',$sectionMenu->id) }}" enctype="multipart/form-data">
			...
			@method('PUT')
						...
						<select class="select2_single form-control" tabindex="-1" name="numero_section">
							...
							@for ($i = 1; $i <= 15; $i++)
								<option value="{{$i}}"
										...
										@if ($sectionMenu->numero_section == $i)
										selected 
										id="selected" 
										@endif
										
									
									@foreach ($menu->section_menus as $sections)
				...
				<input type="number" name="menu_id" value="{{ $sectionMenu->menu_id }}" style="display:none">
				...
				<div class="item form-group">
					...
					<textarea name="contenu" required="required" class="form-control summernote {{ $errors->has('contenu') ? ' is-invalid' : '' }}" cols="30" rows="10">
							{{ old('contenu')??$sectionMenu->section }}
						</textarea>
